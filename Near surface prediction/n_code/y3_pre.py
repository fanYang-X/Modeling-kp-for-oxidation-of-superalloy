...
def cv_prediction(model_name, model, cv_data, train, test, init_test, val_data, save=False):
    ...
    train['factor_pre'] = np.mean(train_pre, axis=1)
    init_test['factor_pre'] = np.mean(test_pre, axis=1)
    ...
    for index, i in enumerate([3, 6, 10, 20, 50, 100]):
        ...
        plt.scatter(val_data[val_data['t'] == i]['distance'], val_data[val_data['t'] == i]['gamapie shape factor'], marker='^', s=50, label='Experimental -{}h'.format(i), c='red')
        plt.plot(val_data[val_data['t'] == i]['distance'], val_data[val_data['t'] == i]['gamapie shape factor'], '--', c='red', alpha=0.8)
        plt.plot(init_test[init_test['t'] == i]['distance'], 
                 init_test[init_test['t'] == i]['factor_pre'], '--', label='{} (TC Com.) Predicted -{}h'.format(model_name, i), linewidth=2.5, c='royalblue')
        plt.fill_between(init_test[init_test['t'] == i]['distance'], init_test[init_test['t'] == i]['factor_pre'] + init_test[init_test['t'] == i]['std'], 
                         init_test[init_test['t'] == i]['factor_pre'] - init_test[init_test['t'] == i]['std'], facecolor='royalblue', alpha=0.3)
        plt.errorbar(train[(train['t'] == i)]['distance'] + val_data[val_data['t'] == i]['distance'].values[1], train[train['t'] == i]['factor_pre'],
        ...
        plt.plot(train[(train['t'] == i)]['distance'] + val_data[val_data['t'] == i]['distance'].values[1], train[train['t'] == i]['factor_pre'], '--', c='tomato', alpha=0.5)
        plt.plot(init_test[init_test['t'] == i]['distance'], 
        ...
        plt.ylabel("γ' shape factor", {'family': 'Times New Roman', 'weight': 'bold', 'size': 9})
        plt.xlim(0, val_data[val_data['t'] == i]['distance'].max() + 2)
        # plt.ylim(0, val_data[val_data['t'] == i]['gamapie size'].max() + 1)
        plt.ylim(0, 1)
        plt.legend(loc=2, frameon=False)
        '''plt.scatter(train[(train['t'] == i)]['distance'] + val_data[val_data['t'] == i]['distance'].values[1], train[train['t'] == i]['factor_pre'], s=50, label='Predicted -{}h ML'.format(i), c='green', alpha=0.5)
        plt.plot(train[(train['t'] == i)]['distance'] + val_data[val_data['t'] == i]['distance'].values[1], train[train['t'] == i]['factor_pre'], '--', c='green', alpha=0.5)
        plt.scatter(val_data[val_data['t'] == i]['distance'], val_data[val_data['t'] == i]['gamapie shape factor'], marker='^', s=50, label='Experimental -{}h'.format(i), c='red')
        plt.plot(val_data[val_data['t'] == i]['distance'], val_data[val_data['t'] == i]['gamapie shape factor'], '--', c='red', alpha=0.8)
        plt.plot(init_test[init_test['t'] == i]['distance'], 
                 init_test[init_test['t'] == i]['factor_pre'], '--', label='{} Predicted -{}h'.format(model_name, i), linewidth=2.5, c='royalblue')
        plt.plot(init_test[init_test['t'] == i]['distance'], 
                 init_test[init_test['t'] == i]['Al'] / 25, '--', label='{} Al -{}h'.format('TC', i), linewidth=2.5, c='green')
        plt.xlabel('Distance (um)', font)
        plt.ylabel("γ' shape factor", font)
        plt.xlim(0, val_data[val_data['t'] == i]['distance'].max() + 2)
        # plt.xlim(0, val_data[val_data['t'] == i]['distance'].max() + 10)
        plt.ylim(0, 1)
        plt.legend(loc=4)'''
        if save:
            plt.savefig('C:/Users/lenovo/Desktop/NSModel/figure/n/{}_n_ML_predicted.tif'.format(model_name), dpi=330, bbox_inches='tight')
    plt.show()

n_path = 'C:/Users/lenovo/Desktop/NSModel/data/NS_data_n.csv'
n_data = pd.read_csv(n_path, encoding='gb2312')
n_data0 = n_data[n_data['alloy'] == '32Y3']
n_data = n_data[n_data['alloy'] != '32Y3']
features = ['distance', 'Ni', 'Al', 'Co', 'Cr', 'Mo', 'Re', 'Ru', 'Ti', 'Ta', 'W', 'Hf', 'Nb']
...
y3_data = pd.read_csv('D:/ML kp/NS_ml_revised/data/y3_n.csv', encoding='gb2312')
...
cv_prediction('RF', rf, cv_data, n_data0, test_data, y3_tc_data, y3_data, True)

## SVR model
svr = SVR(**params['SVR'])
cv_prediction('SVR', svr, cv_data, n_data0, test_data, y3_tc_data, y3_data, True)

## KRR model
krr = KernelRidge(**params['KRR'])
cv_prediction('KRR', krr, cv_data, n_data0, test_data, y3_tc_data, y3_data, True)

## LGB model
LGB = lgb.LGBMRegressor(**params['LGB'], silent=False, verbosity=-1)
cv_prediction('LGB', LGB, cv_data, n_data0, test_data, y3_tc_data, y3_data, True)

## ENET model
enet = ElasticNet(**params['ENET'])
cv_prediction('ENET', enet, cv_data, n_data0, test_data, y3_tc_data, y3_data, True)
