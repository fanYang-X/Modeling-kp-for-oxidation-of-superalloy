...
def cv_prediction(model_name, model, cv_data, train, train_label, test, test_label, save=False):
    ...
    plt.text(0.2, 5.4, s='RMSE: {:.3f}'.format(np.sqrt(mean_squared_error(test_label, test_pre))))
    plt.text(0.2, 4.9, s='MAE: {:.3f}'.format(mean_absolute_error(test_label, test_pre)))
    plt.text(0.2, 4.4, s= '$\mathregular{R^2}$' + ': {:.3f}'.format(r2_score(test_label, test_pre)))
    plt.xlim(0, 7)
    plt.ylim(-0.1, 7)
    ...
    plt.legend()
    if save:
        plt.savefig('C:/Users/chonglh/Desktop/oxidaton2.0/nearsurface/NS_ml/code/s/result/{}_s_test.tif'.format(model_name), dpi=330, bbox_inches='tight')
    plt.show()
